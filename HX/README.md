...
- [Authorization List (QA · Name List only · Section/Team from Name List)](hx.html#auth-list) — [standalone page](authorization-list.html)
- [License Records (CSV dept reference · Name List only)](hx.html#lic-records) — [standalone page](lic-records.html)
...
- **English-only pages:** `hx.html#links` / `#flow` / `#training` / `#auth-list` / `#lic-records` / `#audit` / `#team-board` / `#courses` / `#sections` / `#ocg-pax` / `#brs` (old `#ke-rs` aliases to `#brs`) and those HTML files must be English UI. Staff Chinese names stay. Not this rule: `#docs` / `#tooling` / `#fth-rewrite` / `#scratch` / `#rules`.
